...
export default function MyPageContent() {
  ...
  if (needsLogin || error) {
    ...
    const liffUrl = liffId ? `https://liff.line.me/${liffId}/my` : null;
    return (
        ...
        <Card className="w-full max-w-md text-center">
          <CardContent className="p-8">
            ...
            <p className="mb-6 text-sm text-gray-500">
              此頁面需要透過 LINE 登入，才能查看您的需求紀錄與預約狀態。
            </p>
            <div className="space-y-3">
              {liffUrl && (
                <a
                  href={liffUrl}
                  className="block w-full rounded-lg bg-[#06C755] px-6 py-3 text-sm font-semibold text-white transition hover:bg-[#05a647]"
                >
                  用 LINE 開啟
                </a>
              )}
